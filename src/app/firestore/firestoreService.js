...
export function addEventToFirestore(event) {
  ...
  return db.collection("events").add({
    ...
    }), // you cannot query events related to a users uid from array of obj, that's why we use attendeeIds
...
export async function followUser(profile) {
  const user = firebase.auth().currentUser;
  const batch = db.batch(); // when use batch, the functions below will be batched together, data consistency
  try {
    batch.set(
      db
