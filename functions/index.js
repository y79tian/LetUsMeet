...
exports.addFollowing = functions.firestore
  ...
  .onCreate(async (snapshot, context) => {
    ...
    try {
      ...
      batch.update(db.collection("user").doc(context.params.profileId), {
...
exports.eventUpdated = functions.firestore
  ...
  .onUpdate(async (snapshot, context) => {
    // compare the snapshot before and after update
    const before = snapshot.before.data();
    const after = snapshot.after.data();

    // when someone join the meeting
    ...
    // when someone leave the meeting
    if (before.attendees.length > after.attendees.length) {
